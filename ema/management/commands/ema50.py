...
from ema.models import Ema50Model  # Assuming Ema50Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate EMA50, and store it in the database'
    ...
    def calculate_ema(self, close_prices, period=50):
        """
        ...
        if data:
            ...
            for i in range(len(ema_values)):
                ...
                if pd.isna(ema_value):  # Skip if EMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - EMA50 is NaN")
                    continue
                ...
                # Save to database
                Ema50Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "ema_50_value": ema_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - EMA50: {ema_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate EMA50.")
