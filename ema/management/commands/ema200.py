...
from ema.models import Ema200Model  # Assuming Ema200Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate EMA200, and store it in the database'
    ...
    def calculate_ema(self, close_prices, period=200):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            ema_values = self.calculate_ema(closes, period=200)
            ...
            for i in range(len(ema_values)):
                ...
                if pd.isna(ema_value):  # Skip if EMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - EMA200 is NaN")
                    continue
                ...
                # Save to database
                Ema200Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "ema_200_value": ema_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - EMA200: {ema_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate EMA200.")
