...
from ema.models import Ema100Model  # Assuming Ema100Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate EMA100, and store it in the database'
    ...
    def calculate_ema(self, close_prices, period=100):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            # Calculate EMA100 values
            ema_values = self.calculate_ema(closes, period=100)
            ...
            for i in range(len(ema_values)):
                ...
                if pd.isna(ema_value):  # Skip if EMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - EMA100 is NaN")
                    continue
                ...
                # Save to database
                Ema100Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "ema_100_value": ema_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - EMA100: {ema_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate EMA100.")
