...
from momentum.models import SqueezeModel  # Assuming SqueezeModel exists for storing Squeeze data
from django.db.models import Max

class Command(BaseCommand):
    help = 'Fetch data from DB, calculate SQUEEZ, and store it in the database'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            # Calculate SQUEEZ for each symbol
            for i in range(20, len(closes)):  # Ensure we have enough data for the first period (20)
                ...
                # Save to database
                SqueezeModel.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "squeeze_indicator": squeeze_condition,
                        "signal": signal
                    }
                )
                self.stdout.write(f"Saved {symbol} - SQUEEZ: {signal}")

        except Exception as e:
            self.stdout.write(f"Error calculating SQUEEZ: {str(e)}")
