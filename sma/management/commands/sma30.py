...
from sma.models import Sma30Model  # Assuming Sma30Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate SMA30, and store it in the database'
    ...
    def calculate_sma(self, close_prices, period=30):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            # Calculate SMA30 values
            sma_values = self.calculate_sma(closes, period=30)
            ...
            for i in range(len(sma_values)):
                ...
                if pd.isna(sma_value):  # Skip if SMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - SMA30 is NaN")
                    continue
                ...
                # Save to database
                Sma30Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "sma_30_value": sma_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - SMA30: {sma_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate SMA30.")
