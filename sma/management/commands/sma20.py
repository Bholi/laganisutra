...
from sma.models import Sma20Model  # Assuming Sma20Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate SMA20, and store it in the database'
    ...
    def calculate_sma(self, close_prices, period=20):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            sma_values = self.calculate_sma(closes, period=20)
            ...
            for i in range(len(sma_values)):
                ...
                if pd.isna(sma_value):  # Skip if SMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - SMA20 is NaN")
                    continue
                ...
                # Save to database
                Sma20Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "sma_20_value": sma_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - SMA20: {sma_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate SMA20.")
