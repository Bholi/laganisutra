...
from sma.models import Sma10Model  # Assuming Sma10Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate SMA10, and store it in the database'
    ...
    def calculate_sma(self, close_prices, period=10):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            # Calculate SMA10 values
            sma_values = self.calculate_sma(closes, period=10)
            ...
            for i in range(len(sma_values)):
                ...
                if pd.isna(sma_value):  # Skip if SMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - SMA10 is NaN")
                    continue
                ...
                # Save to database
                Sma10Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "sma_10_value": sma_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - SMA10: {sma_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate SMA10.")
