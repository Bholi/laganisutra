...
from sma.models import Sma5Model  # Assuming Sma5Model exists
from livedata.models import LiveFeedData  # Assuming LiveFeedData is where data is stored


class Command(BaseCommand):
    help = 'Fetch data from DB, calculate SMA5, and store it in the database'
    ...
    def calculate_sma(self, close_prices, period=5):
        """
    ...
    def handle(self, *args, **kwargs):
        ...
        if data:
            ...
            # Calculate SMA5 values
            sma_values = self.calculate_sma(closes, period=5)
            ...
            for i in range(len(sma_values)):
                ...
                if pd.isna(sma_value):  # Skip if SMA is NaN
                    self.stdout.write(f"Skipping {symbols[i]} - SMA5 is NaN")
                    continue
                ...
                # Save to database
                Sma5Model.objects.update_or_create(
                    symbol=symbol,
                    date=timestamp.date(),
                    time=timestamp.time(),
                    defaults={
                        "sma_5_value": sma_value,
                        "signal": signal
                    }
                )

                self.stdout.write(f"Saved {symbol} - SMA5: {sma_value:.4f}, Signal: {signal}")
        else:
            self.stdout.write("No data to calculate SMA5.")
